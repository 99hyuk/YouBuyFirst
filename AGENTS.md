...
이 저장소는 너나사 (YouBuyFirst)를 트랙별로 구현합니다. 현재 기반은 커뮤니티 글 수집, 종목 언급 인식, 반응 방향 분류, Spring Boot 저장까지 이어지는 ingestion MVP입니다.

## 제품 목표

너나사 (YouBuyFirst)는 커뮤니티 반응, 종목별 언급량, 시세/호가, 모의투자 에이전트를 결합해 "개미들의 투자 반응을 읽는 참고 서비스"를 만드는 프로젝트입니다.

현재 구현된 기반은 네이버 종토방과 에펨코리아 주식 게시판에서 신규 글을 수집하고, 국내 주식과 미국 상장 주식/ETF 언급을 인식한 뒤, 각 언급의 반응 방향을 `bullish`, `bearish`, `neutral`로 분류해 저장하는 ingestion 파이프라인입니다.

## 시작 원칙

- 사용자가 `crawl 작업`, `front 작업`, `ops로 Notion 정리`, `agent 트랙 맡아`처럼 말하면 먼저 트랙을 정합니다.
- 이미 대화에 주입된 긴 문서는 터미널로 다시 전문 출력하지 않습니다.
- `AGENTS.md`, `docs/CURRENT_HANDOFF.md`, `docs/DOCUMENTATION_GUIDE.md`는 필요한 섹션만 확인합니다.
- 트랙이 명확하면 담당 트랙 README를 우선 보고, 트랙 경계가 헷갈릴 때만 `docs/workstreams/README.md`를 확인합니다.
...
반박할 때는 전제나 위험, 그대로 진행할 때의 문제, 가능한 대안 1-2개, 사용자가 결정할 지점을 짧게 말합니다.

반드시 의심하고 확인할 영역:

- 공개 배포, 크롤링 정책, 약관/robots 리스크
- Notion 루트, Archive, 주요 DB, 제품 기획 구조 변경
- DB/API 계약, 트랙 경계, PR/문서 보존
- 투자 자문처럼 보일 수 있는 표현
...
- `front/`: Vue 3 + Vite + TypeScript 기반 mock 와이어프레임 shell
...
- 사용자 화면에서는 `감성`보다 `커뮤니티 반응`을 우선합니다. 단일 분석값은 `반응 방향`, 내부 후보 필드는 `reactionDirection`입니다.
...
Notion 루트, 홈카드, 주요 DB 페이지, 제품 기획, 작업 진행, 기술 경험 기록, 에이전트 운영 로그, Archive 변경은 사용자용 정보 구조 변경으로 취급합니다.

- 변경 전 대상 page/database와 child link 보존 여부를 확인합니다.
- 큰 구조 변경은 바로 적용하지 말고 후보안이나 작은 섹션 단위로 진행합니다.
- `replace_content`와 `allow_deleting_content`는 최후 수단입니다.
- 제품 기획에는 PR 이력이나 운영 사고를 섞지 않습니다.
- 변경 후 루트와 핵심 카드 2개 이상을 다시 확인합니다.

## Git/PR 규칙

- 작업 하나는 브랜치 하나와 PR 하나로 처리합니다.
- 브랜치 이름은 기본적으로 `codex/<short-task-name>`입니다.
- 루트 checkout은 main/조율용입니다. 병렬 작업은 `.worktrees/<task>`에서 진행합니다.
- PR 제목은 `[트랙][타입] 명사형 요약`으로 씁니다.
- PR에는 `track:*`, `type:*`, `size:*` 라벨을 붙이고, 필요한 경우에만 `part:*`를 붙입니다.
- 한국어 PR 본문은 UTF-8 no BOM 파일을 만들고 `gh --body-file <path>`로 전달합니다.
...
- 제품 개발/운영 문제, 성능 개선, 품질 개선, 기술 결정은 `docs/ENGINEERING_EVIDENCE_GUIDE.md` 기준으로 개발자 기술 경험 DB에 남깁니다.
