...
| `data` | 종목 인식, 별칭 매칭, 반응 방향, 열기 지수, 개미 심리 지수, 30분 집계 |
...
- 사용자 화면에서는 `커뮤니티 반응`을 우선합니다. 단일 분석값은 `반응 방향`, 커뮤니티 기반 대표 점수는 `개미 심리 지수`, 내부 후보 필드는 `reactionDirection`입니다.
...
- 구현 완료 보고나 구조 설명은 `무엇이 생겼는지`, `어떤 데이터가 어디서 와서 어디로 흐르는지`, `사용자는 어떻게 호출/확인하는지`를 먼저 사람 말로 설명합니다.
- provider, API, 아키텍처가 걸린 작업은 파일 목록보다 provider별 역할, 응답 shape, 호출 예시, 캐시/지연/stale 같은 운영 기준을 먼저 설명합니다.
- 파일명과 명령어는 근거와 재현 경로로만 붙입니다. 파일 목록이 답변의 본문을 대체하지 않게 합니다.
