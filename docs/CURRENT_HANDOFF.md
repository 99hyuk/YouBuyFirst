...
- front 트랙에서 사이트형 대시보드 와이어프레임을 `main`에 반영했고, 추가 대시보드 콘텐츠 보강 PR #43은 draft 상태로 열어 두었습니다. 현재 사용자는 front와 더 협의한 뒤 ready 전환/merge 여부를 판단합니다.
...
- Notion 작업 로그 카드는 PR 본문과 같은 아이콘 섹션 구조를 씁니다. 제목/아이콘만 있는 텍스트 묶음이 아니라 `🧭 한눈에 보기`, `🧩 바뀐 내용`, `🔎 리뷰 가이드`, `📌 범위`, `✅ 검증`, `⚠️ 리스크`, `🗂️ 링크`, `🏷️ 라벨` 순서로 빠르게 훑을 수 있게 작성합니다.
- front 와이어프레임의 정본은 당분간 repo 코드와 문서입니다. Figma AI, Stitch 같은 외부 디자인 도구는 와이어프레임 구조가 어느 정도 굳은 뒤 시안 탐색용으로 쓰고, 최종 선택한 시안만 작은 front PR로 코드에 반영합니다.
...
- Default branch: `main`
- 최근 merge: PR #49 `[ops][docs] 도구 사용 보고 규칙`
- 최근 merge: PR #48 `[crawl][feat] CrawlTarget backend API`
- 최근 merge: PR #47 `[crawl][docs] CrawlTarget queue 설계`
- 최근 merge: PR #46 `[ops][docs] 사용자 이해 중심 완료 보고 규칙`
- 최근 merge: PR #45 `[crawl][feat] 실패 원인별 backoff 정책`
- 최근 merge: PR #44 `[crawl][feat] 정책 skip crawl run 기록`
- 최근 merge: PR #42 `[crawl][feat] CrawlTarget 최소 실행 단위`
- 최근 merge: PR #41 `[front][feat] 사이트형 대시보드 와이어프레임`
...
- 작업 로그 카드는 PR 템플릿과 같은 아이콘 섹션 구조로 작성합니다. 긴 본문도 섹션 아이콘과 짧은 요약으로 먼저 읽히게 만들고, 세부 명령어는 필요할 때만 접힌 영역이나 보조 문장으로 둡니다.
...
- Front dashboard content draft branch: `npm test`, `npm run build`, `git diff --check` 통과. Browser smoke에서 `종목별 심리 비교`, 기간 탭, 카드형 종목 그리드, 원형 지표, 심리 비교 막대 표시와 기존 `반응 터미널` 문구 제거를 확인했습니다.
- Front site polish branch: Vitest 3개 테스트, production build, `git diff --check`, desktop/mobile browser DOM 확인, GitHub Actions Backend/Pipeline 통과
