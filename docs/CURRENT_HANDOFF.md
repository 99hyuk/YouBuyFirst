...
- 종목별 수급은 `GET /api/market/investor-flows/history?symbol=005930.KS&limit=20` 단일 public API로 분리합니다. backend는 `symbol + tradeDate` display cache를 제공하고, pipeline은 pykrx adapter 후보와 한국시간 평일 18:30 refresh job을 둡니다. 단일 최신 수급 public API는 두지 않고, provider 실패 row는 0 수급처럼 저장/공개하지 않습니다. 로컬 pykrx 실 provider 검증은 KRX 응답 문제로 실패할 수 있어 provider 접근 안정화가 다음 리스크입니다.
