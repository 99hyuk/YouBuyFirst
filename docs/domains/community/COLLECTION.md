...
- FMKOREA는 브라우저 렌더링 전용 수집 대상입니다. 로컬 source policy가 허용할 때도 HTTP 목록 요청을 먼저 보내지 않습니다.
...
네이버 종토방 같은 종목형 게시판은 전체 종목을 매번 순회하지 않습니다. `stock-board` `CrawlTarget`으로 분리하고, target 우선순위에 따라 제한적으로 수집합니다.
...
- `NAVER_WATCHLIST_CODES`가 있으면 해당 종목을 우선 넣습니다. 예: `005930,000660,005930.KS`
- 기존 `NAVER_STOCK_CODES`는 같은 의미의 호환 설정으로 유지하되, `NAVER_WATCHLIST_CODES`가 있으면 watchlist 설정을 우선합니다.
- 설정 관심종목 뒤에는 코드에 고정한 대형주와 대표 ETF 후보를 붙입니다.
- `NAVER_STOCK_TARGET_LIMIT` 기본값은 25개이고, 운영 실수 방지를 위해 30개를 넘기지 않습니다.
- 기본 후보는 전체 국내 종목 master를 펼치지 않습니다. 임의의 신규 종목은 일반 게시판 언급 급증, 관심종목, 포트폴리오, 시장 영향 후보로 들어올 때만 깊이 수집합니다.
- 관심종목 target은 `priority=80`, `crawlIntervalSeconds=1800`입니다. 기본 대형주/ETF target은 `priority=120`, `crawlIntervalSeconds=3600`입니다.
