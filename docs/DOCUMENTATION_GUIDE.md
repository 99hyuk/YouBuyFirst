...
새 채팅이나 새 에이전트는 아래 문서의 필요한 섹션만 먼저 확인합니다. 전문 출력은 기본값이 아닙니다.

| 문서 | 역할 | 확인 원칙 |
| --- | --- | --- |
| `AGENTS.md` | 공통 작업 규칙과 금지 사항 | 필요한 섹션 |
| `docs/CURRENT_HANDOFF.md` | 현재 상태, 최근 결정, 다음 후보 | 필요한 섹션 |
| `docs/CHAT_START_GUIDE.md` | 빈 채팅, 짧은 트랙 지시, 작업자 응답 틀 | 필요한 경우 |
| `docs/workstreams/README.md` | 트랙 선택 기준 | 트랙 경계가 헷갈릴 때 |
| 담당 트랙 README | 맡은 트랙의 파일 소유권과 범위 | 작업 시작 시 관련 섹션 |
...
작업이 끝났을 때 모든 문서를 고치지 않습니다. 아래 기준으로 필요한 문서만 갱신하되, `CURRENT_HANDOFF.md`에는 오래 남을 현재 상태만 한두 줄로 반영합니다.
