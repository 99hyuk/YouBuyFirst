...
현재 단계의 정본은 `front/` 코드와 repo 문서입니다. 즉 Codex가 만든 와이어프레임은 단순 그림이 아니라 라우팅, mock data, 화면 문구, API 후보, `기획자 확인 필요` 항목을 함께 담은 제품 계약입니다.

Figma AI, Stitch 같은 외부 디자인 도구는 이 계약을 대체하지 않습니다. 쓰는 시점은 아래처럼 나눕니다.

1. `front/`에서 정보 구조와 화면 밀도를 먼저 잡습니다.
2. 사용자가 주요 섹션, 용어, 데이터 우선순위에 동의하면 외부 디자인 도구에 현재 화면 구조를 참고 자료로 넘겨 시안을 탐색합니다.
3. 마음에 드는 시안이 나오면 색, 간격, 타이포그래피, 카드 밀도, 차트 스타일만 추출해 작은 front PR로 코드에 반영합니다.
4. 외부 도구 산출물은 참고 시안이고, 최종 정본은 merge된 `front/` 코드와 PR/Notion 기록입니다.

처음부터 Figma/Stitch에서 와이어프레임을 만들고 repo로 가져오는 방식은 지금 단계의 기본값이 아닙니다. 그러면 API 후보, mock fixture, 테스트, 기획자 확인 항목이 코드와 분리되어 나중에 다시 맞춰야 합니다. 다만 브랜드 톤이나 시각 스타일 후보를 빠르게 비교하고 싶을 때는 외부 도구를 보조로 사용할 수 있습니다.

디자인 PR로 넘어가기 전에는 아래 조건을 확인합니다.

- 화면 용어가 한국어 사용자에게 어색하지 않음
- 첫 화면에서 무엇을 판단할 수 있는지 분명함
- mock data의 지표 의미와 실제 API 후보가 분리되어 있음
- 모바일/데스크톱에서 카드 밀도와 차트가 깨지지 않음
- gstack 또는 브라우저 smoke로 화면, 콘솔, 반응형을 확인함
