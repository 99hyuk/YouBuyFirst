...
- 현재 API 예시: `005930.KS`, `NVDA`. 현재가/등락률/거래량은 `GET /api/quotes`, 메인 차트는 `GET /api/market/chart-candles`, 국내 수급은 `GET /api/market/investor-flows`를 사용한다.
- Child screens:
...
- 일별 수급 영역: 국내 종목/ETF는 `GET /api/market/investor-flows?symbols=005930.KS` 응답으로 개인/외국인/기관의 거래일별 순매수/순매도 금액과 수량을 표로 표시한다. `tradeDate`, `provider`, `sourceLabel`, `delayLabel`, `asOf`, `stale`, `dataStatus`를 함께 표시하고, 미국 종목이거나 `dataStatus`가 `INSUFFICIENT`, `PROVIDER_ERROR`, `MOCK`이면 이 영역만 숨긴다. 현재 API가 1거래일만 내려주면 1줄만 보이고, 다일 수급 API가 붙으면 5줄 높이 스크롤 목록으로 확장한다.
...
- investor flow stale/error: 수급은 거래일별 확정 관찰 데이터라 장중 실시간처럼 보이게 표시하지 않는다. `investorFlow.delayLabel`, `investorFlow.tradeDate`, `investorFlow.provider`, `investorFlow.stale`, `investorFlow.dataStatus`를 수급 영역에 함께 표시한다. `PROVIDER_ERROR`, `MOCK`, `INSUFFICIENT`는 0 수급처럼 보이지 않게 수급 표를 숨긴다.
...
| `investorFlow.symbol`, `name`, `market`, `currency`, `tradeDate`, `provider`, `sourceLabel`, `delayLabel`, `asOf`, `stale`, `dataStatus`, `individual`, `foreign`, `institution` | market | `GET /api/market/investor-flows` 응답 shape. 국내 종목/ETF의 거래일별 개인/외국인/기관 수급만 다루며, `dataStatus`가 `INSUFFICIENT`, `PROVIDER_ERROR`, `MOCK`이면 수급 영역을 숨긴다. |
| `investorFlow.individual.netAmount`, `netVolume`, `foreign.netAmount`, `netVolume`, `institution.netAmount`, `netVolume` | market | 거래일별 순매수/순매도 금액과 수량. 양수는 순매수, 음수는 순매도 방향이며 투자 판단 문구로 쓰지 않는다. |
...
- investor flow는 `GET /api/market/investor-flows`를 사용한다. 현재 provider adapter는 pykrx 후보이며, provider 접근 안정화 전까지 `PROVIDER_ERROR`가 올 수 있다.
- SimplyStock처럼 일별 매매동향을 5줄 이상 스크롤로 보여주려면 market API가 한 종목의 여러 `tradeDate` 행을 내려줘야 한다. 현재 프론트 표는 여러 행을 받을 수 있게 준비되어 있지만, API가 1거래일만 내려주면 1줄만 표시한다.
...
- 2026-05-22: 국내 종목/ETF 수급은 `GET /api/market/investor-flows` 별도 API로 연결했다. 가격/차트 API에는 개인/외국인/기관 수급을 넣지 않고, 해외 종목이나 `INSUFFICIENT`, `PROVIDER_ERROR`, `MOCK` 응답은 수급 표만 숨긴다.
