...
- 전 거래일 수급 영역: 국내 종목/ETF는 `GET /api/market/investor-flows?symbols=005930.KS` 응답으로 개인/외국인/기관의 전 거래일 순매수/순매도 금액과 수량을 표시한다. `tradeDate`, `provider`, `sourceLabel`, `delayLabel`, `asOf`, `stale`, `dataStatus`를 함께 표시하고, 미국 종목이거나 `dataStatus`가 `INSUFFICIENT`, `PROVIDER_ERROR`, `MOCK`이면 이 영역만 숨긴다.
...
| `investorFlow.individual.netAmount`, `netVolume`, `foreign.netAmount`, `netVolume`, `institution.netAmount`, `netVolume` | market | 전 거래일 순매수/순매도 금액과 수량. 양수는 순매수, 음수는 순매도 방향이며 투자 판단 문구로 쓰지 않는다. |
...
- 2026-05-22: 국내 종목/ETF 전 거래일 수급은 `GET /api/market/investor-flows` 별도 API로 연결했다. 가격/차트 API에는 개인/외국인/기관 수급을 넣지 않고, 해외 종목이나 `INSUFFICIENT`, `PROVIDER_ERROR`, `MOCK` 응답은 수급 카드만 숨긴다.
