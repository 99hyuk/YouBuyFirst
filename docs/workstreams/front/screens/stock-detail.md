...
- 현재 API 예시: `005930.KS`, `NVDA`. 현재가/등락률/거래량은 `GET /api/quotes`, 메인 차트는 `GET /api/market/chart-candles`, 국내 수급은 `GET /api/market/investor-flows/history`를 사용한다.
- Child screens:
...
| `investorFlow.symbol`, `name`, `market`, `currency`, `tradeDate`, `provider`, `sourceLabel`, `delayLabel`, `asOf`, `stale`, `dataStatus`, `individual`, `foreign`, `institution` | market | `GET /api/market/investor-flows/history` 응답 shape. 국내 종목/ETF의 거래일별 개인/외국인/기관 수급만 다루며, `dataStatus`가 `OK`, `STALE`인 row만 수급 표에 표시한다. |
...
- 2026-05-22: 국내 종목/ETF 수급은 `GET /api/market/investor-flows/history` 별도 API로 연결했다. 가격/차트 API에는 개인/외국인/기관 수급을 넣지 않고, 해외 종목이나 빈 history 응답은 수급 표만 숨긴다.
